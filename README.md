...
- **WASM**: the official prebuilt Emscripten transcoder (`basis_universal/webgl/transcoder`) is bundled by vite into a single self-contained ES module (`javascript/index.es.js`): the wasm binary is inlined as a data URL and transcoding runs in a dedicated inline Web Worker. The bundle is committed and embedded into the Rust wasm via `include_str!`, then loaded at runtime through a Blob + dynamic `import()`. Consumers need no extra toolchain, file serving, or copy steps.
...
- `TargetFormat` is a `repr(u32)` enum whose values match `basist::transcoder_texture_format` (e.g. `Rgba32 = 13`).
...
npm run build   # syncs vendored artifacts, vite build, writes javascript/index.es.js
```
